  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4">
      <div className="flex items-center gap-2">
        ...
        <button className="btn-primary btn-sm" onClick={() => setAdding(true)} disabled={!services.length}>+ {t("waitlist.add")}</button>
      </div>
      <p className="muted">{t("waitlist.intro")}</p>
      {withMatches.length > 0 && <p className="rounded-xl bg-green-50 p-3 text-sm font-medium text-green-800">✨ {t("waitlist.matches_found", { count: withMatches.length })}</p>}
          ...
          <div key={r.id} className={`card space-y-3 ${r.expired ? "opacity-60" : ""}`}>
            <div className="flex flex-wrap items-start gap-2">
              <div className="mr-auto">
                <Link href={`/customers/${r.customer_id}`} className="font-semibold hover:underline">{r.customer?.first_name} {r.customer?.last_name}</Link>
            ...
            {r.expired ? (
              ...
              <div>
                <p className="mb-1.5 text-xs font-medium text-green-700">{t("waitlist.available_now")}</p>
                <div className="flex flex-wrap gap-1.5">
                  {r.matches.map((m) => (
                    <button
                      key={m.start}
                      disabled={pending}
                      className="rounded-lg border border-green-300 bg-green-50 px-2.5 py-1 text-sm first-letter:uppercase text-green-900 hover:bg-green-100"
                      onClick={() =>
                ...
                <p className="mt-2 rounded-xl bg-stone-50 p-2.5 text-sm text-stone-700">{msg}</p>
              </div>
            ) : (
            ...
            <div className="flex flex-wrap gap-2">
              {msg && <button className="btn-secondary btn-sm" onClick={() => navigator.clipboard?.writeText(msg)}>{t("message.copy")}</button>}
              {wa && <a className="btn-secondary btn-sm" href={wa} target="_blank" rel="noreferrer" onClick={() => run(() => setWaitlistStatus(r.id, "contacted"))}>{t("message.open_whatsapp")}</a>}
              ...
              <button className="btn-ghost btn-sm text-red-600" disabled={pending} onClick={() => run(() => setWaitlistStatus(r.id, "cancelled"))}>{t("waitlist.remove")}</button>
            </div>
          </div>
        );
      })}
      ...
      <div>
        ...
        {customer ? (
          ...
          <>
            ...
            />
            {results.length > 0 && q && (
              <ul className="mt-1 divide-y divide-stone-100 rounded-xl border border-stone-200">
                {results.map((c) => (
