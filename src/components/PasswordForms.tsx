  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-sm flex-col justify-center px-4 py-10">
      ...
      <div className="card">
        ...
        <p className="muted mb-5">{subtitle}</p>
        {children}
      </div>
    </div>
  );
...
export function ForgotPasswordForm() {
  ...
  return (
    <Shell title={t("auth.forgot_title")} subtitle={t("auth.forgot_subtitle")}>
      {state?.info ? (
        <p className="rounded-xl bg-green-50 p-3 text-sm text-green-800">{t(state.info as MessageKey)}</p>
      ) : (
        <form action={action} className="space-y-4">
          ...
          {state?.error && <p className="text-sm text-red-600">{t(state.error as MessageKey)}</p>}
          <button className="btn-primary w-full" disabled={pending}>{pending ? t("common.loading") : t("auth.send_reset")}</button>
        </form>
      )}
      <p className="mt-5 text-center text-sm">
        <Link className="font-semibold text-brand-600" href="/login">← {t("auth.login")}</Link>
      </p>
    </Shell>
  );
}
    ...
    <Shell title={t("auth.reset_title")} subtitle={t("auth.reset_subtitle")}>
      {!hasSession ? (
        <div className="space-y-4">
          <p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-900">{t("auth.reset_link_invalid")}</p>
